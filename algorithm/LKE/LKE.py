...
class LogParser:
    ...
    def writeResultToFile(self):
        ...
        for i in glob(os.path.join(self.para.savePath, "template[0-9]*.txt")):
            os.remove(i)
        for i in range(len(self.df_log)):
            template_index = template_map[self.df_log["EventId"][i]]
            with open(os.path.join(self.para.savePath, "template%d.txt"%template_index), "a") as f:
                f.write(str(self.df_log["LineId"][i])+"\n")
        open(os.path.join(self.para.savePath, "logTemplates.txt"), "w").write("\n".join(df_event["EventTemplate"])+"\n")
    ...
    def generate_logformat_regex(self, logformat):
        ...
        for k in range(len(splitters)):
            if k % 2 == 0:
                ...
                splitter = re.sub(r' +', lambda _: '\s+', splitters[k])
                regex += splitter
            else:
