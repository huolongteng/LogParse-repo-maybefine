...
class LogParser:
    ...
    def outputResult(self, logClustL):
        ...
        # changed to keep the same format with logTIM
        template_map = dict(zip(list(df_event['EventId']), list(range(1, len(df_event['EventId'])+1))))
        for i in glob(os.path.join(self.savePath, "template[0-9]*.txt")):
            os.remove(i)
        for i in range(len(self.df_log)):
            template_index = template_map[self.df_log["EventId"][i]]
            with open(os.path.join(self.savePath, "template%d.txt"%template_index), "a") as f:
                f.write(str(self.df_log["LineId"][i])+"\n")
        # df_event.to_csv(os.path.join(self.savePath, "logTemplates.txt"), columns=["EventTemplate"], index=False, header=False, sep=" ", quoting=csv.QUOTE_NONE, quotechar='', escapechar='')
        open(os.path.join(self.savePath, "logTemplates.txt"), "w").write("\n".join(df_event["EventTemplate"])+"\n")
    ...
    def parse(self, logname, update=True):
        ...
        if not update:
            ...
            for idx, line in self.df_log.iterrows():
                logID = line['LineId']
                msg_tokens = list(filter(lambda x: x != '', re.split(r'[\s=:,]', self.preprocess(line['Content']))))
                best_tpl = templates[0] if templates else 'NoMatch'
                best_score = -1
                for tpl in templates:
                    tpl_tokens = tpl.split()
                    score = sum(1 for a, b in zip(tpl_tokens, msg_tokens) if a == '*' or a == b)
                    if score > best_score:
                        best_score = score
                        best_tpl = tpl
